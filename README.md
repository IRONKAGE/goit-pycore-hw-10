...
По-перше, додамо додатковий функціонал до класів із попередньої домашньої роботи:
...
- Додайте й адаптуйте до класу `AddressBook` нашу функцію з четвертого домашнього завдання, тиждень 3, `get_upcoming_birthdays`, яка для контактів адресної книги повертає список користувачів, яких потрібно привітати по днях на наступному тижні.
...
Для реалізації нового функціоналу також додайте функції-обробники з такими командами:

- `add-birthday` — додаємо до контакту день народження у форматі `DD.MM.YYYY`
...
Тож у фіналі наш бот повинен підтримувати такий список команд:

1. `add [ім'я] [телефон]`: Додати або новий контакт з іменем і телефонним номером, або телефонний номер до контакту, який уже існує.
...
Для прикладу розглянемо реалізацію команди `add [ім'я] [телефон]`. У функції `main` ми повинні додати обробку цієї команди у відповідне місце:
...
Сама реалізація функції `add_contact` може виглядати таким чином:
...
2. Усі дані повинні виводитися у зрозумілому та зручному для користувача форматі.

3. Усі помилки, як-от неправильний ввід чи відсутність контакту, повинні оброблятися інформативно з відповідним повідомленням для користувача.
